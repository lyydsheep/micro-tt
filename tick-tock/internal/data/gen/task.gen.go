...
func newTask(db *gorm.DB, opts ...gen.DOOption) task {
	...
	_task.taskDo.UseModel(&biz.Task{})

	tableName := _task.taskDo.TableName()
	_task.ALL = field.NewAsterisk(tableName)
	_task.ID = field.NewInt64(tableName, "id")
	_task.App = field.NewString(tableName, "app")
	_task.Tid = field.NewString(tableName, "tid")
	_task.Output = field.NewString(tableName, "output")
	_task.RunTime = field.NewTime(tableName, "run_time")
	_task.CostTime = field.NewInt64(tableName, "cost_time")
...
type task struct {
	...
	RunTime    field.Time   // 执行时间
	CostTime   field.Int64  // 执行耗时（毫秒）
	Status     field.Int32  // 当前状态 未执行 0 执行中 1 执行成功 2 执行失败 3
...
func (t *task) updateTableName(table string) *task {
	...
	t.CostTime = field.NewInt64(table, "cost_time")
...
func (t *task) fillFieldMap() {
	...
	t.fieldMap["cost_time"] = t.CostTime
...
func (t taskDo) Create(values ...*biz.Task) error {
	if len(values) == 0 {
		return nil
	}
...
func (t taskDo) CreateInBatches(values []*biz.Task, batchSize int) error {
	return t.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (t taskDo) Save(values ...*biz.Task) error {
	if len(values) == 0 {
		return nil
	}
...
func (t taskDo) First() (*biz.Task, error) {
	if result, err := t.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*biz.Task), nil
	}
}

func (t taskDo) Take() (*biz.Task, error) {
...
func (t taskDo) Last() (*biz.Task, error) {
...
func (t taskDo) Find() ([]*biz.Task, error) {
	result, err := t.DO.Find()
	return result.([]*biz.Task), err
}

func (t taskDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*biz.Task, err error) {
	buf := make([]*biz.Task, 0, batchSize)
...
func (t taskDo) FindInBatches(result *[]*biz.Task, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (t taskDo) FirstOrInit() (*biz.Task, error) {
...
func (t taskDo) FirstOrCreate() (*biz.Task, error) {
...
func (t taskDo) FindByPage(offset int, limit int) (result []*biz.Task, count int64, err error) {
...
func (t taskDo) Delete(models ...*biz.Task) (result gen.ResultInfo, err error) {
